...
import { transformPriceQueryResponse, filterDataWithDateRange } from './price-query-transformer.util';
  ...
  switch (action.type) {
    case PriceQueryActionTypes.PriceQueryFetched: {
      return priceQueryAdapter.addAll(
        filterDataWithDateRange(transformPriceQueryResponse(action.queryResults), action.fromDate, action.toDate),
        state
      );
    }
